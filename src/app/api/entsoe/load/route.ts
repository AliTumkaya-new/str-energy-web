...
import { fetchActualLoad } from "@/lib/entsoeApi";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await fetchActualLoad(params.country, params.startDate, params.endDate);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[api/entsoe/load]", error);
