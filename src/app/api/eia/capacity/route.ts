...
import { fetchInstalledCapacity } from "@/lib/eiaApi";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await fetchInstalledCapacity(params.country, params.startYear, params.endYear);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[api/eia/capacity]", error);
