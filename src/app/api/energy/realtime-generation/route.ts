...
const DEFAULT_ENDPOINT = "https://seffaflik.epias.com.tr/electricity-service/v1/generation/data/realtime-generation";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[api/energy/realtime-generation] EPIAS upstream failed", { status: response.status, body: errorBody.slice(0, 500) });

      if (response.status === 400 && /mevcut değil|not available|SEF1124/i.test(errorBody)) {
        return NextResponse.json({ items: [], notice: "Seçilen tarih aralığı için veri henüz yayınlanmadı." });
      }
